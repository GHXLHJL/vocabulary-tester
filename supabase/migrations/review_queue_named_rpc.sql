...
create or replace function public.fetch_review_aggregates(p_secret text, p_review_status text)
returns table (
    aggregate_key text,
    word text,
    word_key text,
...
grant execute on function public.fetch_review_aggregates(text, text) to anon, authenticated;
grant execute on function public.apply_review_decision(text, text, text, text, text) to anon, authenticated;
